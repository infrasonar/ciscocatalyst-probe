...
def addr_netmask(addr: bytes, n: int):
    try:
...
def ip_mib_address(orig_key: str, item: dict[str, Any]):
    ...
    key = tuple(map(int, orig_key.split('.')))
    try:
        local_typ = key[0]
